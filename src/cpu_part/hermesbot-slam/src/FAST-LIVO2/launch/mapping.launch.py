...
def generate_launch_description():
    ...
    static_publisher = os.path.join(get_package_share_directory("fast_livo"), "scripts", "static_extrin_publisher.py")

    #Load parameters
    avia_config_cmd = os.path.join(config_file_dir, "hermes.yaml")
    camera_config_cmd = os.path.join(config_file_dir, "camera_realsense_d435.yaml")
    frames_config_cmd = os.path.join(config_file_dir, "frames.yaml")
    ...
    frames_config_arg = DeclareLaunchArgument(
        'frames_params_file',
        default_value=frames_config_cmd,
        description='Full path to frames config (frames.map/base/cloud/etc.)',
    )
    ...
    frames_params_file = LaunchConfiguration('frames_params_file')
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=[
                "python3", static_publisher,
                "--ros-args",
                "--params-file", avia_params_file,
                "--params-file", frames_params_file,
            ],
            output="screen",
        )
    ])
