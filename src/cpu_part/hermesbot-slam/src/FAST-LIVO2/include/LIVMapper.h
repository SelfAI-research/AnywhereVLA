...
struct FramesCfg {
  std::string map, base, lidar, camera, imu, cloud, path, image, imu_prop_world;
};
...
public:
  ...
  void publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager, const rclcpp::Time& stamp);
  ...
  void publish_effect_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list, const rclcpp::Time& stamp);
  void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pmavros_pose_publisherubOdomAftMapped, const rclcpp::Time& stamp);
