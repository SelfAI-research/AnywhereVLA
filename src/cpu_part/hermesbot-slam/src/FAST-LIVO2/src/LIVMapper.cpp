      ...
      extR(M3D::Identity())
{
  ...
  tf_br_ = std::make_unique<tf2_ros::TransformBroadcaster>(this->node);
  ...
  path.header.frame_id = frames_.path;
}
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<int>("preprocess.scan_line", 16);
  try_declare.template operator()<double>("preprocess.downsample_rate", 0.0);
  ...
  try_declare.template operator()<std::string>("frames.map", "camera_init");
  try_declare.template operator()<std::string>("frames.base", "aft_mapped");
  try_declare.template operator()<std::string>("frames.lidar", "lidar");
  try_declare.template operator()<std::string>("frames.camera", "camera");
  try_declare.template operator()<std::string>("frames.imu", "imu_link");
  try_declare.template operator()<std::string>("frames.cloud", "");          // fallback to map
  try_declare.template operator()<std::string>("frames.path", "");           // fallback to map
  try_declare.template operator()<std::string>("frames.image", "");          // fallback to map
  try_declare.template operator()<std::string>("frames.imu_prop_world", "world");
  ...
  this->node->get_parameter("preprocess.downsample_rate", p_pre->downsample_rate);
  ...
  this->node->get_parameter("frames.map", frames_.map);
  this->node->get_parameter("frames.base", frames_.base);
  this->node->get_parameter("frames.lidar", frames_.lidar);
  this->node->get_parameter("frames.camera", frames_.camera);
  this->node->get_parameter("frames.imu", frames_.imu);
  this->node->get_parameter("frames.cloud", frames_.cloud);
  this->node->get_parameter("frames.path",  frames_.path);
  this->node->get_parameter("frames.image", frames_.image);
  this->node->get_parameter("frames.imu_prop_world", frames_.imu_prop_world);

  if (frames_.cloud.empty()) frames_.cloud = frames_.map;
  if (frames_.path.empty())  frames_.path  = frames_.map;
  if (frames_.image.empty()) frames_.image = frames_.map;
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  sub_pcl = this->node->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, 100, std::bind(&LIVMapper::standard_pcl_cbk, this, std::placeholders::_1));
  sub_imu = this->node->create_subscription<sensor_msgs::msg::Imu>(imu_topic, rclcpp::SensorDataQoS(), std::bind(&LIVMapper::imu_cbk, this, std::placeholders::_1));
  sub_img = this->node->create_subscription<sensor_msgs::msg::Image>(img_topic, 100, std::bind(&LIVMapper::img_cbk, this, std::placeholders::_1));
...
void LIVMapper::processImu() 
{
  ...
  // std::cout << "[ Mapping ] predict sta: " << state_propagat.pos_end.transpose() << "  vel: "<< state_propagat.vel_end.transpose() << std::endl;
}
...
void LIVMapper::handleVIO() 
{
  ...
  const rclcpp::Time stamp = sec2Stamp(LidarMeasures.measures.back().vio_time);
  publish_frame_world(pubLaserCloudFullRes, vio_manager, stamp);
  publish_img_rgb(pubImage, vio_manager, stamp);
}
...
{    
  ...
  // std::cout << "feats_down_body->points.size() = " << feats_down_body->points.size() << std::endl;
  ...
  rclcpp::Time stamp = sec2Stamp(LidarMeasures.last_lio_update_time);  // scan end / LIO time
  publish_odometry(pubOdomAftMapped, stamp);
  ...
  *pcl_w_wait_pub = *laserCloudWorld;
  ...
  if (!img_en) publish_frame_world(pubLaserCloudFullRes, vio_manager, stamp);
  if (pub_effect_point_en) publish_effect_world(pubLaserCloudEffect, voxelmap_manager->ptpl_list_, stamp);
  if (voxelmap_manager->config_setting_.is_pub_plane_map_) voxelmap_manager->pubVoxelMap();
  publish_path(pubPath, stamp);
  publish_mavros(mavros_pose_publisher, stamp);
  ...
  RCLCPP_INFO(
    ...
    "\n"
    "\033[1;34m+-------------------------------------------------------------+\033[0m\n"
    "\033[1;34m|     LIO Average Time             \033[1;36m| %-25f\033[1;34m|\033[0m\n"
    "\033[1;34m+-------------------------------------------------------------+\033[0m\n",
    aver_time_consu
  );
}
...
void LIVMapper::imu_prop_callback()
{
  ...
  if (imu_prop_enable && !prop_imu_buffer.empty())
  {
    ...
    imu_prop_odom.header.frame_id = frames_.imu_prop_world;
  ...
  // after imu_propagate is updated:
  if (imu_prop_enable) 
  {
      tf2::Transform tf;
      tf.setOrigin(tf2::Vector3(
          imu_propagate.pos_end(0),
          imu_propagate.pos_end(1),
          imu_propagate.pos_end(2)
      ));

      Eigen::Quaterniond q_eigen(imu_propagate.rot_end);
      tf2::Quaternion q_tf(q_eigen.x(), q_eigen.y(), q_eigen.z(), q_eigen.w());
      tf.setRotation(q_tf);

      geometry_msgs::msg::TransformStamped ts =
          createTransformStamped(tf, newest_imu.header.stamp, frames_.map, frames_.base);
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  if (!lidar_en) return;
  mtx_buffer.lock();
  // RCLCPP_INFO(this->node->get_logger(), "get Lidar at time: %.6f", stamp2Sec(msg->header.stamp));
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO:
  {
    ...
    if (!lidar_pushed)
    {
      ...
      // Assumption: all points are sorted by curvature
      for (size_t i = 1; i < meas.lidar->points.size(); ++i) {
        assert(meas.lidar->points[i-1].curvature <= meas.lidar->points[i].curvature && "Lidar points must be sorted by curvature");
      }
      
      auto msg_timestamp = lid_header_time_buffer.front();
      meas.lidar_frame_beg_time = msg_timestamp + meas.lidar->points.front().curvature; // calc lidar scan start time
      meas.lidar_frame_end_time = msg_timestamp + meas.lidar->points.back().curvature; // calc lidar scan end time

      // Normilize points time
      auto first_point_time = meas.lidar->points[0].curvature;
      for (size_t i = 0; i < meas.lidar->points.size(); ++i) {
        meas.lidar->points[i].curvature -= first_point_time;
      }

      
      RCLCPP_INFO(this->node->get_logger(),
          "[LIDAR]\n\t  timestamp: %.6f,\n\t begin time: %.6f,\n\t end   time: %.6f",
          msg_timestamp, meas.lidar_frame_beg_time, meas.lidar_frame_end_time);
      // RCLCPP_INFO(this->node->get_logger(),
      //     "\t point front: %.6f, point back:  %.6f",
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      double lid_newest_time = lid_header_time_buffer.back() + lid_raw_data_buffer.back()->points.back().curvature;
  ...
  case ONLY_LO:
  {
    ...
    { 
      ...
      meas.lidar = lid_raw_data_buffer.front(); // push the first lidar topic
      meas.lidar_frame_beg_time = lid_header_time_buffer.front() + meas.lidar->points.front().curvature; // generate lidar_beg_time
      meas.lidar_frame_end_time  = lid_header_time_buffer.front() + meas.lidar->points.back().curvature; // calc lidar scan end time
      lidar_pushed = true;             
    }
    struct MeasureGroup m; // standard method to keep imu message.
    m.lio_time = meas.lidar_frame_end_time;
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager, const rclcpp::Time& stamp)
{
  ...
  laserCloudmsg.header.frame_id = frames_.cloud;
...
void LIVMapper::publish_effect_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list, const rclcpp::Time& stamp)
{
  ...
  laserCloudFullRes3.header.frame_id = frames_.cloud;
...
{
  odomAftMapped.header.frame_id = frames_.map;
  odomAftMapped.child_frame_id = frames_.base;
  ...
  tf_br_->sendTransform(geometry_msgs::msg::TransformStamped(createTransformStamped(transform, odomAftMapped.header.stamp, frames_.map, frames_.base)));
...
void LIVMapper::publish_mavros(const rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr &mavros_pose_publisher, const rclcpp::Time& stamp)
{
  ...
  msg_body_pose.header.frame_id = frames_.map;
...
void LIVMapper::publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr &pubPath, const rclcpp::Time& stamp)
{
  set_posestamp(msg_body_pose.pose);
  msg_body_pose.header.stamp = stamp;
  msg_body_pose.header.frame_id = frames_.path;
