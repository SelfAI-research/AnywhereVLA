...
void ImuProcess::Forward_without_imu(LidarMeasureGroup &meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  const double &pcl_end_time = pcl_beg_time + pcl_out.points.back().curvature;
  meas.last_lio_update_time = pcl_end_time;
  const double &pcl_end_offset_time = pcl_out.points.back().curvature;
  ...
  if (lidar_type != L515)
  {
    ...
    {
        dt_j= pcl_end_offset_time - it_pcl->curvature;
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger(""), "[ IMU ] undistort input size: %zu", lidar_meas.pcl_proc_cur->points.size());
  RCLCPP_INFO(rclcpp::get_logger(""), "[ IMU ] IMU data sequence size: %zu", meas.imu.size());
  // RCLCPP_INFO(rclcpp::get_logger(""), "[ IMU ] lidar_scan_index_now: %d", lidar_meas.lidar_scan_index_now);
  RCLCPP_INFO(rclcpp::get_logger(""), "[ IMU ] imu_beg_time: %f", imu_beg_time);
  RCLCPP_INFO(rclcpp::get_logger(""), "[ IMU ] imu_end_time: %f", imu_end_time);
